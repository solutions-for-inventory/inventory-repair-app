...
module Graphql.InventoryPart.Persistence (
...
import Graphql.Part.Persistence (itemFilters)
import Enums
...
getInventoryItemPredicate item Predicate {..} | T.strip field == "" || (T.strip operator) `P.elem` ["", "in", "like"] || T.strip value == "" = []
                                              ...
                                              | T.strip field == "partId" = [getOperator operator (item ^. InventoryPart_PartId) (E.val (toSqlKey $ fromIntegral $ parseToInteger $ T.strip value))]
...
getInventoryItemInPredicate item Predicate {..} | T.strip operator /= "in" || T.strip value == "" = []
                                                ...
                                                | T.strip field == "partId" = [(item ^. InventoryPart_PartId) `in_` (E.valList $ fromText (\ e -> toSqlKey $ fromIntegral $ parseToInteger $ T.strip e) value)]
...
getInventoryItemNotInPredicate item Predicate {..} | T.strip operator /= "notIn" || T.strip value == "" = []
                                                   ...
                                                   | T.strip field == "partId" = [(item ^. InventoryPart_PartId) `notIn` (E.valList $ fromText (\ e -> toSqlKey $ fromIntegral $ parseToInteger $ T.strip e) value)]
...
inventoryItemQueryCount page =  do
                      result  <- runDB
                                   ...
                                   $ E.from $ \ (item `E.InnerJoin` inventoryItem) -> do
                                     E.on $ item ^. Part_Id E.==. inventoryItem ^. InventoryPart_PartId
...
createOrUpdateInventoryItems :: [InventoryPartArg] -> Handler [InventoryPart_Id]
...
createOrUpdateInventoryItem :: InventoryPartArg -> Handler InventoryPart_Id
createOrUpdateInventoryItem inventoryItem = do
                            let InventoryPartArg {..} = inventoryItem
                            now <- liftIO getCurrentTime
                            itemEntityId <- if inventoryItemId > 0 then
                                        do
                                         let inventoryItemKey = (toSqlKey $ fromIntegral $ inventoryItemId)::InventoryPart_Id
--                                         let inventoryItemKey = InventoryPart_Key {unInventoryPart_Key  = partId}
...
--                                                                     , InventoryPart_PartId =. ((toSqlKey $ fromIntegral partId)::Part_Id)
...
fromInventoryItemQL :: InventoryPartArg -> UTCTime -> Maybe UTCTime -> InventoryPart_
fromInventoryItemQL (InventoryPartArg {..}) cd md = InventoryPart_ { inventoryPart_Code = code
                                                                   , inventoryPart_Level = level
                                                                   , inventoryPart_MaxLevelAllowed = maxLevelAllowed
                                                                   , inventoryPart_MinLevelAllowed = minLevelAllowed
                                                                   , inventoryPart_Price = realToFrac price
                                                                   , inventoryPart_Location = location
